...
import dev.akif.battleships.api.Coordinates
import dev.akif.battleships.api.GameResponse
import dev.akif.battleships.api.PlaceShipRequest
...
class GameController(private val games: GameService) {
    ...
    fun create(): Mono<ResponseEntity<GameResponse>> =
        games.create().map {
            ResponseEntity
                .created(URI.create("/games/${it.id}"))
                .body(it.toResponse())
        }
